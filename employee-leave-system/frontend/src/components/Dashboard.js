...
function Dashboard({ user, logout }) {
  ...
  useEffect(() => {
    ...
    const loadAllLeaves = () => {
      LeaveService.getAllLeaves()
        ...
        .catch((err) => console.error("Error loading all leaves:", err));
    };
  ...
  const addLeave = (savedLeave) => {
    const normalized = {
      ...savedLeave,
      employeeName: savedLeave.employeeName || username,
    ...
    setAllHistory((prev) => [...prev, normalized]);
  };
  ...
  const pendingLeaves = allHistory.filter(
      ...
      >
            ...
            >
              ...
              <p style={{ fontSize: "28px", fontWeight: "800" }}>
                ...
                <span
                  style={{
                    background:
                      "linear-gradient(90deg,#ffeb3b,#00e5ff,#ff4081)",
        ...
        {isAdmin && (
          <div>
            ...
            >
              {showAllHistory ? "Current Leaves" : "All History"}
            </button>
            <LeaveHistory
