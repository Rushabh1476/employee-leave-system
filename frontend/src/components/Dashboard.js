...
function Dashboard({ user, logout }) {
  ...
  useEffect(() => {
    if (isEmployee && currentUser?.username) {
      LeaveService.getEmployeeLeaves(currentcurrentcurrentUser.username).then((res) => {
        const deletedIds = getLS("EMP_DELETED_IDS");
        const data = (res.data || []).map((l) => ({ ...l, id: l.id, employeeName: l.employeeName || currentcurrentcurrentUser.username, date: l.date || "", status: l.status || "Pending", proof: l.proof })).filter((l) => !deletedIds.includes(l.id));
        setLeaves(data);
      }).catch(console.error);
    }
  ...
  const addLeave = (savedLeave) => {
    const normalized = { ...savedLeave, employeeName: savedLeave.employeeName || currentcurrentcurrentUser.username, date: savedLeave.date || "", status: savedLeave.status || "Pending", proof: savedLeave.proof };
        ...
        {isEmployee && (
          <div style={{ textAlign: "center" }}>
            <div style={{ padding: "28px", borderRadius: "16px", marginBottom: "25px", background: "linear-gradient(135deg,#2c3e50,#4ca1af)", color: "#fff" }}>
              ...
              <p style={{ fontSize: "28px", fontWeight: "800" }}>Welcome <span style={{ background: "linear-gradient(90deg,#ffeb3b,#00e5ff,#ff4081)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{currentcurrentcurrentUser.username}??</span></p>
            </div>
        ...
        {isAdmin && (
          <div>
            <div style={{ padding: "22px", borderRadius: "12px", marginBottom: "25px", background: "#ffffff", border: "1px solid #e0e0e0", textAlign: "center" }}>
              ...
              <p>Welcome <strong>{currentcurrentcurrentUser.username}</strong></p>
            </div>
